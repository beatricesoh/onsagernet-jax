...
class OnsagerNetHD2(SDE):
    ...
    def dissipation(self, x: ArrayLike)  -> Array:
        ...
        M_x = sqrtdissipation(x, args=None)
        return M_x @ M_x.T
...
class OnsagerNetHD2_V2(SDE):
    ...
    def dissipation(self, x: ArrayLike, args: ArrayLike=None)  -> Array:
        """Dissipation matrix wrapper
        ...
        """
        sqrtdissipation = self.shared()[1]
        M_x = sqrtdissipation(x, args)
        return M_x @ M_x.T

    def _matrix_div(self, M: eqx.Module, x: ArrayLike, args: ArrayLike=None) -> Array:
        r"""Computes the matrix divergence of a matrix function $M(x)$.
